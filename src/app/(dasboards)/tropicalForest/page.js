...
export default function TropicalForest(){
  ...
  const [userAllJournals, setUserAllJournal] = useState([]);
  ...
  const getCurrentUserJournals = async() => {
    ...
    const deleteTask = async (journals) => {
        try {
            ...
            const journalDoc = querySnapshot.docs.find(doc => doc.data().journalTitle === journals.journalTitle && doc.data().createdBy === journals.createdBy);
  ...
  if(!user){
    ...
    return(
        <main className="min-h-screen w-full flex select-none">
          <div className="md:w-[20%] md:block hidden bg-green-950 py-6 px-2">
          ...
          <div className="md:w-[80%] w-full forestBG flex justify-center items-center relative p-3">
          <h1 className="text-xl md:text-3xl absolute md:top-9 top-28 left-10 md:left-20">Hey {user?.displayName}👋 welcome to the forest 🌳</h1>
            <nav className="p-5 top-0 right-0 absolute">
             <Popover>
              <PopoverTrigger>
              <img src={user?.photoURL} className="rounded-full h-16 w-16 cursor-pointer border-4 border-green-600" alt="user" />
              </PopoverTrigger>
              <PopoverContent>
            ...
            <div className="block space-y-4">
              <h1 className="text-3xl text-center text-green-400 md:mt-4 mt-24">Your Journals 📖</h1>
              ...
              {journal.map((j) => (
                <div key={j.journalId} className="cardJournal rounded-xl border border-white border-opacity-20 p-3 space-y-4 h-auto w-full">
                  <div className="flex justify-between gap-6 px-4">
                    ...
                    <h1 className="rounded-[30px] md:px-3 px-2 py-1 text-gray-300 bg-lime-800">{j.journalMood}</h1>
                  </div>
                  <div className="block space-y-4">
                    <h1 className="text-lime-300 text-xl">{j.journalTitle}</h1>
                    <p className="text-lime-400 text-sm">{j.journalDes}</p>
                  </div>
                  <Popover>
                    <PopoverTrigger>
